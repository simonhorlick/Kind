...
// - Tid: type-level identity
// - Utt: the type of non-terminating terms
// - Utv: wraps a term as non-terminating
// - Ute: unwraps a non-terminating term
...
const Lam = (name, bind, body, eras, loc) => ["Lam", {name, bind: bind||Hol(), body, eras}, MEMO && ("lm" + (eras?"~":"") + body[2])          , loc];
...
const Hol = (name, loc)                   => ["Hol", {name: name||new_name()},              MEMO && ("{?" + name + "?}")                      , loc];
...
const reduce = (term, opts = {}) => {
  ...
  const unquote = (term, names = null) => {
    ...
    switch (ctor) {
      ...
      case "Lam": return Lam(term.name, unquote(term.bind, names), x => unquote(term.body, names_ext(x, null, names)), term.eras);
...
const erase = (term) => {
  ...
  switch (c) {
    ...
    case "Lam": return t.eras ? f(subst(t.body, e, 0)) : Lam(t.name, Hol(), f(t.body), t.eras);
  ...
  const typecheck = (term, expect, ctx = ctx_new, affine = true, lvel = 0) => {
    ...
    switch (term[0]) {
      ...
      case "Typ":
        ...
        break;
      case "Tid":
        var expr_t = typecheck(term[1].expr, Typ(), ctx, false, lvel);
        type = Typ();
        break;
      case "Utt":
      ...
      case "Utv":
        ...
        var expr_t = typecheck(term[1].expr, expr_t, ctx, false, lvel);
        type = Utt(expr_t);
        break;
      case "Ute":
        if (affine) {
        ...
        var expr_t = typecheck(term[1].expr, null, ctx, false, lvel);
      ...
      case "All":
        ...
        var bind_t = typecheck(term[1].bind, Typ(), ctx, false, lvel);
        var ex_ctx = ctx_ext(term[1].name, null, term[1].bind, term[1].eras, false, lvel, ctx);
        var body_t = typecheck(term[1].body, Typ(), ex_ctx, false, lvel);
        type = Typ();
        break;
      case "Lam":
        ...
        var bind_t = typecheck(bind_v, Typ(), ctx, false, lvel);
        var ex_ctx = ctx_ext(term[1].name, null, bind_v, term[1].eras, false, lvel, ctx);
        var body_t = typecheck(term[1].body, expect_nf && expect_nf[0] === "All" ? expect_nf[1].body : null, ex_ctx, affine, lvel);
        var body_T = typecheck(body_t, Typ(), ex_ctx, false, lvel);
      ...
      case "Slf":
        ...
        var type_t = typecheck(term[1].type, Typ(), ex_ctx, false, lvel);
        type = Typ();
        break;
      case "New":
        ...
        var ttyp_t = typecheck(ttyp, null, ctx, false, lvel);
        var expr_t = typecheck(term[1].expr, subst(ttyp[1].type, Ann(ttyp, term, true), 0), ctx, affine, lvel);
        type = term[1].type;
        break;
      case "Use":
        var expr_t = typecheck(term[1].expr, null, ctx, affine, lvel);
        var expr_t = weak_normal(expr_t);
        if (expr_t[0] !== "Slf") {
      ...
      case "Ann":
        if (!term[1].done) {
          ...
          try {
            var type_t = typecheck(term[1].type, Typ(), ctx, affine, lvel);
            var expr_t = typecheck(term[1].expr, term[1].type, ctx, affine, lvel);
            type = term[1].type;
          } catch (e) {
            term[1].done = false;
            throw e;
      ...
      case "Log":
        ...
        try {
          var msge_t = typecheck(msge_v, null, ctx, false, lvel);
        ...
        var expr_t = typecheck(term[1].expr, expect, ctx, affine);
        type = expr_t;
        break;
      case "Hol":
      ...
      case "Ref":
        if (!(opts.defs||{})[term[1].name]) {
          ...
          var dref_t = typecheck((opts.defs||{})[term[1].name], null, ctx, affine, lvel);
    ...
    if (expect) {
      ...
      if (expect_nf[0] === "Utt" && type_nf[0] !== "Utt") {
        return typecheck(Utv(term), expect_nf, ctx_arg, affine)
      }
      // Fill an Ute
      if (expect_nf[0] !== "Utt" && type_nf[0] === "Utt") {
        return typecheck(Ute(term), expect_nf, ctx_arg, affine)
      }
...
module.exports = {
  ...
  App, Slf, New, Use, Ann, Log, Hol, Ref,
  equal,
  erase,
  reduce,
  shift,
