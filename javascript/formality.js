...
const show_context = (ctx, i = 0) => {
  var bind = get_bind(ctx, i);
  if (bind) {
    var term = " : " + (bind[1] ? show(norm(bind[1], {}, false), ctx) : "?");
...
const norm = (foo, defs = {}, weak = false, dup = false) => {
  ...
  const apply = (eras, func, argm) => {
    ...
    // ([x = a] b c) ~> [x = a] (b c)
      ...
      return norm(Dup(func[1].name, func[1].expr, App(func[1].body, shift(argm, 1, 0), eras)), defs, weak, dup);
    } else {
  ...
  const duplicate = (name, expr, body) => {
    ...
    // [x = [y = a] b] c ~> [y = a] [x = b] c
      ...
      return norm(Dup(expr[1].name, expr[1].expr, Dup(name, expr[1].body, shift(body, 1, 0))), dup); 
    } else {
  ...
  const dereference = (eras, name) => {
    if (defs[name]) {
      var nf = norm(defs[name].term, defs, weak, dup);
  ...
  switch (ctor) {
    ...
    case "Box": return Box(term.name, cont(term.expr, defs, weak, dup));
    case "Put": return dup ? norm(term.expr, defs, weak, dup) : Put(term.name, term.type && cont(term.expr, defs, weak, dup, dup), cont(term.expr, defs, weak, dup));
...
const infer = (term, defs, ctx = Ctx(), strat = true) => {
  ...
  switch (term[0]) {
    ...
    case "App":
      var func_t = norm(infer(term[1].func, defs, ctx, strat), defs, true);
      if (func_t[0] !== "All") {
    ...
    case "Dup":
      var expr_t = norm(infer(term[1].expr, defs, ctx, strat), defs, true);
      if (expr_t[0] !== "Box") {
...
const show_mismatch = (expect, actual, expr, ctx) => {
  ...
  text += "- Expect = " + show(norm(expect, {}, false), ctx) + "\n";
  text += "- Actual = " + show(norm(actual, {}, false), ctx) + "\n"
  text += "\n[CONTEXT]\n" 
